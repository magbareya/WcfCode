...
namespace STS
{
    class SecurityTokenService : IWSTrust
    {
        ...
        static X509SecurityToken GetSTSToken()
        {
            X509Certificate2 cert = new X509Certificate2();
            cert.Import(@"STS.pfx", "1234", X509KeyStorageFlags.DefaultKeySet);
        ...
        /// <param name="subjectName">The subject name of the certificate to use as the basis for the security token</param>
        /// <returns>An X509SecurityToken</returns>
        static X509SecurityToken GetServiceToken()
        {
            X509Certificate2 cert = new X509Certificate2();            
            cert.Import(@"Server Public.cer");
            return new X509SecurityToken(cert);
        }
